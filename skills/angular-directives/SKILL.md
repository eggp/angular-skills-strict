...
Use `@let` to declare local variables in your templates, which is especially useful for caching signal values to avoid repeated execution or unwrapping observables with async pipe just once.

```html
<!-- Caching a signal value -->
...
<!-- Complex calculation -->
@let total = calculateTotal(items(), taxRate());
<div class="summary">
  <p>Subtotal: {{ items() | sum }}</p>
  <p>Total (w/ tax): {{ total }}</p>
</div>
```
